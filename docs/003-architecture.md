...
| -- | ----- | ------------ |
| `traceId` | Whole flight trail — one bloom/nectar chain | runtime / bus event |
| `agentId` | Single adapter invocation (one `agent` process) | runtime (ULID) |
...
internal/
  ...
  runtime/                  # dispatch: colony → prompts → adapter
```
