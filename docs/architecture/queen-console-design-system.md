...
- A **route-scoped store** owns a different endpoint and starts on mount / stops on unmount, so an idle console keeps no poll running. `tracesStore` polls `/api/traces` (15s); `traceStore` polls `/api/traces/:id` (10s) for the detail route; `gitStore` polls `/api/git` (15s), which is the slowest read in the console because every one of its polls shells out to `git` several times. The rule is one poller *per endpoint*, not one poller per app.
