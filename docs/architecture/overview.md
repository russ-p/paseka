...
Trail comb (trace-scoped handoff files; gitignored with `runs/`):

```
.paseka/runs/<traceId>/artifacts/
├── research.md        # bee-written draft notes (example)
└── review-comments.md # human Console comb write (017 Slice B)
```

Bees receive `{{.ArtifactsDir}}` (absolute path to the comb). Runtime announces **added/changed** comb files on successful AFK or interactive exit via `SIGNAL/artifact.written` (SHA-256 delta vs per-run baseline under `<agentId>/artifacts-baseline.json`). Human producers (`console`) write and publish immediately. Distinct from adapter `result.json` artifact blobs and Object Store mutation refs — see [Spec 014](../specs/014-artifacts-protocol.md).
...
| `params.model` | `--model <id>` (resolved vendor id after `model_aliases` lookup) |
...
5. **Stream JSON** — stdout when `output_format: stream-json` (lifecycle/diagnostic parse only; domain events are not extracted from assistant text). When the final `result` line includes `usage` (`inputTokens`, `outputTokens`, `cacheReadTokens`, `cacheWriteTokens`), the adapter persists it on `result.json` as optional `usage` (source `cursor.stream-json`). When stream (or compact JSON) lines include `session_id`, the adapter persists **`providerSessionId`** on `result.json` and `meta.json` so later export/inspect can associate the run with Cursor’s native chat without copying the vendor store. `paseka export --include agent-logs` resolves a per-run **Agent log** through `SessionLogResolver` at read time: Cursor reads `~/.cursor/projects/<slug>/agent-transcripts/<uuid>/<uuid>.jsonl` (UUID = `providerSessionId`; workspace slug first, then glob). Missing provider data never fails export. Pi still stubs that seam. Adapters without usage omit the field; Honey Reserve (`energyToken`) stays dispatch-count based and is unrelated.
...
Interactive runs add `session.json` and `transcript.ndjson` under the same `.paseka/runs/<traceId>/<agentId>/` tree. Cursor HITL pre-creates a chat (`agent create-chat`) and launches the TUI with `--resume`; OpenCode HITL pre-creates a session (`opencode serve` + `POST /session`, no model turn), launches the TUI with `--session` plus its own loopback control server, and posts the kickoff to `/session/<id>/prompt_async`. **Resume** skips pre-create for both and reuses the stored `providerSessionId` on a new Paseka session (`resumedFrom` on meta). Pi HITL pins `--session-id <agentId>`. The native id is stored as **`providerSessionId`** on `session.json` and `meta.json` before the PTY starts (missing id does not fail a new session). Active sessions are registered in `~/.config/paseka/<slug>/state.json`. Terminal UI (default terminal vs Ghostty) is configured in `~/.config/paseka/<slug>/terminal.yaml`.
