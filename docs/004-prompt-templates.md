...
| Variable | Set by |
| -------- | ------ |
| `Bee`, `TraceID`, `AgentID`, `ColonyRoot`, `Workspace`, `Task`, `Insights`, `ResultFile` | `internal/runtime.Dispatcher` at dispatch time |
...
Shipped by `paseka init` as `.paseka/prompts/_partials/json-events.md`. Instructs the agent to emit bus-compatible NDJSON lines:

```json
{"traceId":"<traceId>","type":"INSIGHT","payload":{"summary":"..."}}
```
