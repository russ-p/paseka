...
For `review: final` / `_review`, Queen Console Reviews shows an accumulated worktree merge preview (`GET /api/traces/:traceId/merge-diff`) before approve; **Request changes** on the preview submits annotated comments via `POST .../reject` with a `comments` array and starts rework ([spec 017](../specs/017-console-diff-review.md)). See [specs/002-queen-console-mvp.md](../specs/002-queen-console-mvp.md).
