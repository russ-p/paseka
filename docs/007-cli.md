...
Seed offline with `invite record`, or publish `SIGNAL/session.invite` while `paseka run` is up (reactor projects to `state.json`). Accept publishes `beekeeper.ready` and starts grilling on the **same** `traceId`.
