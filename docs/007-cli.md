...
Ideation `SIGNAL` kinds (`feature.requested`, `feature.classified`, `spec.ready`) are **colony choreography contracts** — field shapes are defined in the spec and bee emit partials, not enforced by `internal/protocol`. Platform HITL kinds (`session.invite`, `beekeeper.ready`) are validated at the invite boundary (`paseka invite record`, `event emit`, accept/reject).
...
Invite statuses: `pending`, `accepted`, `completed`, `incomplete`, `cancelled`, `deferred`. Grilling invites become `completed` when a bus event matches the invite's `done_when` contract (default: `spec.ready` with `ref` on disk); `incomplete` when the session ends without a valid artifact.

Seed offline with `invite record`, or configure **`auto_invites`** in `.paseka/colony.yaml` so `paseka run` auto-publishes when a matching bus event arrives (default: `feature.classified` with `decision=grill`). Accept publishes `beekeeper.ready` and starts grilling on the **same** `traceId`.
...
# With paseka run up: invite list shows pending grilling invite (no invite record needed)
