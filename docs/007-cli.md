...
Ideation `SIGNAL` kinds (`feature.requested`, `feature.classified`, `spec.ready`) are **colony choreography contracts** — field shapes are defined in the spec and bee emit partials, not enforced by `internal/protocol`. `paseka signal` checks envelope only (`traceId`, domain `type`, valid JSON with `payload.kind`). Platform HITL kinds (`session.invite`, `beekeeper.ready`) will be validated at the invite boundary when Phase 2 lands.
