...
Invite statuses: `pending`, `accepted`, `completed`, `incomplete`, `cancelled`, `deferred`. Grilling invites become `completed` when an `invite_completion` rule matches (default: `spec.ready` with `ref` on disk); `incomplete` when the session ends without a valid artifact.
