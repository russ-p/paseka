...
Invite statuses: `pending`, `accepted`, `completed`, `incomplete`, `cancelled`, `deferred`. Grilling invites become `completed` when `spec.ready` arrives and `ref` exists on disk; `incomplete` when the session ends without a valid spec.

Seed offline with `invite record`, or configure **`auto_invites`** in `.paseka/colony.yaml` so `paseka run` auto-publishes when a matching bus event arrives (default: `feature.classified` with `route=grill`). Accept publishes `beekeeper.ready` and starts grilling on the **same** `traceId`.
