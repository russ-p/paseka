...
- **Summary:** Migrate the remaining `/next` routes (Timeline, Tasks, Reviews, Sessions, Bees, Worktrees, Runs, Topology, Settings) off `PagePlaceholder`, then cut the root over from the legacy bundle. Blocked per route on the components already landed: `Section`/`MetaList`/`DetailRow` for detail pages, `Drawer` for the Sessions launch and Tasks create forms, `DataTable` for the remaining lists.
