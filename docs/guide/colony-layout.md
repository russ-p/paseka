...
Each `traceId` shares one **Honey Reserve** (`energyToken`): every adapter dispatch (`task.ready` and direct routing) consumes one token. When the reserve is empty, tasks move to `blocked` with summary `Honey reserve exhausted`. Beekeepers can top up a live tick via `paseka energy add --trace <id> --amount <n>`. Per-cue **initial** reserve overrides on fresh trails; standing cues then replace remaining each tick from `standing.stipend`: [Forage Cues](cues.md) § Honey.
