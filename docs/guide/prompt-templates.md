...
| `{{.TraceTitle}}` | `string` | Resolved human Flight Trail title. From latest `INSIGHT/trace.title`, then `feature.requested` title, then first task title; empty when unresolved. |
| `{{.WorktreeBranch}}` | `string` | Resolved git branch for the trace worktree. From latest `INSIGHT/worktree.branch`, else `paseka/<traceId>`. |
...
| Variable | Set by |
| -------- | ------ |
| `Bee`, `TraceID`, `TraceTitle`, `AgentID`, `TaskID`, `ColonyRoot`, `Workspace`, `Task`, `Intent`, `IntentRaw`, `Insights`, `ResultFile`, `ArtifactsDir`, `Interactive`, `IsLastWorkTask`, `Adapter` | `internal/runtime.Dispatcher` at dispatch time; `Interactive` is `true` in `internal/sessions` for chat |
...
| `ArtifactsDir` | `.paseka/runs/<traceId>/artifacts/` (created lazily) |
...
| `scout` | `emit-howto`, `emit-insight`, `emit-signal`; on `intake` also `scout-emit-intake` |
