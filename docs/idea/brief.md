...
- **Эмерджентная стабильность**: Защита от бесконечных циклов ИИ с помощью затухающих токенов энергии (`energyToken`). Фильтрация по уровню уверенности (`confidence`) остаётся исторической идеей и [отложена](../plans/backlog.md), а не является текущим поведением рантайма.
